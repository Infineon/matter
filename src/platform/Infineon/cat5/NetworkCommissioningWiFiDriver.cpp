...
CHIP_ERROR CYW955913WiFiDriver::Init(NetworkStatusChangeCallback * networkStatusChangeCallback)
{
    CHIP_ERROR err;
    size_t ssidLen         = 0;
...
void CYW955913WiFiDriver::Shutdown()
{
    mpStatusChangeCallback = nullptr;
}

CHIP_ERROR CYW955913WiFiDriver::CommitConfiguration()
{
...
CHIP_ERROR CYW955913WiFiDriver::RevertConfiguration()
{
    mStagingNetwork = mSavedNetwork;
    return CHIP_NO_ERROR;
}

bool CYW955913WiFiDriver::NetworkMatch(const WiFiNetwork & network, ByteSpan networkId)
{
    return networkId.size() == network.ssidLen && memcmp(networkId.data(), network.ssid, network.ssidLen) == 0;
}

Status CYW955913WiFiDriver::AddOrUpdateNetwork(ByteSpan ssid, ByteSpan credentials, MutableCharSpan & outDebugText,
...
Status CYW955913WiFiDriver::RemoveNetwork(ByteSpan networkId, MutableCharSpan & outDebugText, uint8_t & outNetworkIndex)
{
    outDebugText.reduce_size(0);
    outNetworkIndex = 0;
...
Status CYW955913WiFiDriver::ReorderNetwork(ByteSpan networkId, uint8_t index, MutableCharSpan & outDebugText)
{
    outDebugText.reduce_size(0);
...
CHIP_ERROR CYW955913WiFiDriver::ConnectWiFiNetwork(const char * ssid, uint8_t ssidLen, const char * key, uint8_t keyLen)
{
    CHIP_ERROR err = CHIP_NO_ERROR;
    ReturnErrorOnFailure(ConnectivityMgr().SetWiFiStationMode(ConnectivityManager::kWiFiStationMode_Disabled));
    // Set the wifi configuration
    wifi_config_t wifi_config;
    chip::DeviceLayer::Internal::CYW955913Utils::populate_wifi_config_t(
    ...
    err = chip::DeviceLayer::Internal::CYW955913Utils::cyw955913_wifi_set_config(WIFI_IF_STA, &wifi_config);
    SuccessOrExit(err);
...
void CYW955913WiFiDriver::OnConnectWiFiNetwork()
{
...
void CYW955913WiFiDriver::ConnectNetwork(ByteSpan networkId, ConnectCallback * callback)
{
    CHIP_ERROR err          = CHIP_NO_ERROR;
    ...
    ChipLogProgress(NetworkProvisioning, "CYW955913 NetworkCommissioningDelegate: SSID: %s", mStagingNetwork.ssid);
...
BitFlags<app::Clusters::NetworkCommissioning::WiFiSecurityBitmap> CYW955913WiFiDriver::ConvertSecuritytype(cy_wcm_security_t security)
{
...
void CYW955913WiFiDriver::scan_result_callback(cy_wcm_scan_result_t * result_ptr, void * user_data, cy_wcm_scan_status_t status)
{
    if ((status == CY_WCM_SCAN_COMPLETE) || (NumAP >= kWiFiMaxNetworks))
    {
        /* Sent Scan response after Scan is complete */
        CYW955913WiFiDriver::GetInstance().OnScanWiFiNetworkDone();
    }
    else
    {
        if (result_ptr != NULL)
        {
            ...
            scan_result_list[NumAP].security = static_cast<cy_wcm_security_t>(
                CYW955913WiFiDriver::GetInstance().ConvertSecuritytype(scan_result_list[NumAP].security).Raw());
            NumAP++;
        } /* end of if ( result_ptr != NULL ) */
    }     /* end of else */
}

CHIP_ERROR CYW955913WiFiDriver::StartScanWiFiNetworks(ByteSpan ssid)
{
    cy_rslt_t result;
...
void CYW955913WiFiDriver::OnScanWiFiNetworkDone()
{
    ...
    if (!ap_num)
    {
        ...
        {
            if (CHIP_NO_ERROR == DeviceLayer::SystemLayer().ScheduleLambda([]() {
    ...
    if (ScanResult)
    {
        if (CHIP_NO_ERROR == DeviceLayer::SystemLayer().ScheduleLambda([ap_num, ScanResult]() {
                CYW955913ScanResponseIterator iter(ap_num, ScanResult);
                if (GetInstance().mpScanCallback)
                {
...
void CYW955913WiFiDriver::ScanNetworks(ByteSpan ssid, WiFiDriver::ScanCallback * callback)
{
    if (callback != nullptr)
    {
...
void CYW955913WiFiDriver::OnNetworkStatusChange()
{
    Network configuredNetwork;
    bool staEnabled = false, staConnected = false;
    VerifyOrReturn(CYW955913Utils::IsStationEnabled(staEnabled) == CHIP_NO_ERROR);
    ...
    VerifyOrReturn(CYW955913Utils::IsStationConnected(staConnected) == CHIP_NO_ERROR);
    if (staConnected)
    {
...
CHIP_ERROR CYW955913WiFiDriver::SetLastDisconnectReason(int32_t reason)
{
    mLastDisconnectedReason = reason;
    return CHIP_NO_ERROR;
}

int32_t CYW955913WiFiDriver::GetLastDisconnectReason()
{
    return mLastDisconnectedReason;
}

size_t CYW955913WiFiDriver::WiFiNetworkIterator::Count()
{
    return mDriver->mStagingNetwork.ssidLen == 0 ? 0 : 1;
}

bool CYW955913WiFiDriver::WiFiNetworkIterator::Next(Network & item)
{
