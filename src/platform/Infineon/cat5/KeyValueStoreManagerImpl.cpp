...
/**
 *    @file
 *          Platform-specific key value storage implementation for CYW955913
 */
...
uint16_t convert_string_to_uint16(const char* str) {
    int c;
    uint16_t hash = 5381;  // Initial value for the hash

    while ((c = *str++)) {
        hash = ((hash << 5) + hash) + c; // Hashing algorithm
    }

    return hash % MTB_KVSTORE_MAX_KEY_VAL;
}
                                          ...
                                          size_t offset_bytes) const
{
    ...
    uint16_t ukey = convert_string_to_uint16(key);

    // Get the value size
    result = mtb_kvstore_read_numeric_key(const_cast<mtb_kvstore_t *>(&kvstore_obj), ukey, NULL, &actual_size);
    ...
    // Read the value
    result = mtb_kvstore_read_numeric_key(const_cast<mtb_kvstore_t *>(&kvstore_obj), ukey, local_value, &size);
...
CHIP_ERROR KeyValueStoreManagerImpl::_Put(const char * key, const void * value, size_t value_size)
{
    ...
    {
        result = mtb_kvstore_write_numeric_key(&kvstore_obj, ukey, NULL, 0, overwrite_key);
    }
    else
    {
        result = mtb_kvstore_write_numeric_key(&kvstore_obj, ukey, (uint8_t *) value, static_cast<size_t>(value_size), overwrite_key);
    }
...
CHIP_ERROR KeyValueStoreManagerImpl::_Delete(const char * key)
{
    ...
    cy_rslt_t result = mtb_kvstore_read_numeric_key(&kvstore_obj, ukey, NULL, NULL);
    if (result == CY_RSLT_SUCCESS)
    {
        result = mtb_kvstore_delete_numeric_key(&kvstore_obj, ukey);
    }
