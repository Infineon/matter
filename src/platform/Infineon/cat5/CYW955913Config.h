...
/**
 * Provides functions and definitions for accessing device configuration information on the CYW955913.
 *
 ...
 */
class CYW955913Config
{
public:
    struct Key;

    // Maximum length of a key name
    static constexpr size_t kMaxConfigKeyNameLength = MTB_KVSTORE_MAX_KEY_VAL;
...
inline CHIP_ERROR CYW955913Config::Key::to_str(char * buf, size_t buf_size) const
{
    if (buf_size < len() + 1)
    {
...
inline size_t CYW955913Config::Key::len() const
{
    // + 1 for separating ';'
...
inline bool CYW955913Config::Key::operator==(const Key & other) const
{
