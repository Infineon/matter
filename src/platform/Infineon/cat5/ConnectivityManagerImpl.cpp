...
#error "WiFi Station support must be enabled when building for CYW955913"
#endif
...
ConnectivityManager::WiFiStationMode ConnectivityManagerImpl::_GetWiFiStationMode(void)
{
    ...
    mWiFiStationMode = (Internal::CYW955913Utils::wifi_get_mode(curWiFiMode) == CHIP_NO_ERROR &&
...
{
    return Internal::CYW955913Utils::IsStationProvisioned();
}
...
void ConnectivityManagerImpl::_ClearWiFiStationProvision(void)
{
    if (mWiFiStationMode != kWiFiStationMode_ApplicationControlled)
    {
        ...
        Internal::CYW955913Utils::cyw955913_wifi_set_config(WIFI_IF_STA, &stationConfig);
...
CHIP_ERROR ConnectivityManagerImpl::_Init()
{
    ...
    ChipLogProgress(DeviceLayer, "ConnectivityManagerImpl : Init Enable Station Mode..");

    // Ensure that CYW955913 station mode is enabled.
    err = Internal::CYW955913Utils::EnableStationMode();
    SuccessOrExit(err);
    ...
    if (!IsWiFiStationProvisioned())
    {
        ...
        if (CHIP_DEVICE_CONFIG_DEFAULT_STA_SSID[0] != 0)
        {
            ...
            err                     = Internal::CYW955913Utils::cyw955913_wifi_set_config(WIFI_IF_STA, &wifiConfig);
            SuccessOrExit(err);
    ...
    err = Internal::CYW955913Utils::SetAPMode(false);
    SuccessOrExit(err);
...
void ConnectivityManagerImpl::wlan_event_cb(cy_wcm_event_t event, cy_wcm_event_data_t * event_data)
{
    ...
    case CY_WCM_EVENT_CONNECTED:
        ...
        NetworkCommissioning::CYW955913WiFiDriver::GetInstance().OnConnectWiFiNetwork();
    ...
    case CY_WCM_EVENT_DISCONNECTED:
        ...
        NetworkCommissioning::CYW955913WiFiDriver::GetInstance().SetLastDisconnectReason(WLC_E_SUP_DEAUTH);
        break;
...
void ConnectivityManagerImpl::ChangeWiFiStationState(WiFiStationState newState)
{
    if (mWiFiStationState != newState)
    {
        ...
        SystemLayer().ScheduleLambda([]() { NetworkCommissioning::CYW955913WiFiDriver::GetInstance().OnNetworkStatusChange(); });
    }
}
...
CHIP_ERROR ConnectivityManagerImpl::ConfigureWiFiAP()
{
    ...
    err = Internal::CYW955913Utils::cyw955913_wifi_set_config(WIFI_IF_AP, &wifiConfig);
    SuccessOrExit(err);

    err = Internal::CYW955913Utils::cyw955913_start_ap();
    if (err != CHIP_NO_ERROR)
    {
        ChipLogError(DeviceLayer, "cyw955913_start_ap failed: %s", chip::ErrorStr(err));
    }
...
void ConnectivityManagerImpl::DriveAPState()
{
    ...
    // Determine if AP mode is currently enabled in the CYW955913 WiFi layer.
    err = Internal::CYW955913Utils::IsAPEnabled(APModeEnabled);
    SuccessOrExit(err);
    ...
    if (mWiFiAPMode != kWiFiAPMode_ApplicationControlled)
    {
        ...
        if (mWiFiAPState != targetState)
        {
            ...
            // a SYSTEM_EVENT_AP_START event will be received from the CYW955913 WiFi layer which will
            // cause the state to transition to 'Active'.
            if (targetState == kWiFiAPState_Active)
            {
                if (mWiFiAPState != kWiFiAPState_Active)
                {
                    err = Internal::CYW955913Utils::SetAPMode(true);
                    SuccessOrExit(err);
                    err = ConfigureWiFiAP();
                    SuccessOrExit(err);
            ...
            else
            {
                if (mWiFiAPState != kWiFiAPState_Deactivating)
                {
                    err = Internal::CYW955913Utils::SetAPMode(false);
                    SuccessOrExit(err);
                    err = Internal::CYW955913Utils::cyw955913_stop_ap();
                    SuccessOrExit(err);
                    ChangeWiFiAPState(kWiFiAPState_Deactivating);
                }
            }
...
exit:
    if (err != CHIP_NO_ERROR && mWiFiAPMode != kWiFiAPMode_ApplicationControlled)
    {
        ...
        Internal::CYW955913Utils::SetAPMode(false);
        Internal::CYW955913Utils::cyw955913_stop_ap();
    }
}
...
void ConnectivityManagerImpl::DriveStationState()
{
    ...
    if (mWiFiStationMode != kWiFiStationMode_ApplicationControlled)
    {
        // Ensure that the CYW955913 WiFi layer is started.
        err = WiFi_init();
        SuccessOrExit(err);

        // Ensure that station mode is enabled in the CYW955913 WiFi layer.
        err = Internal::CYW955913Utils::EnableStationMode();
        SuccessOrExit(err);
    }

    // Determine if the CYW955913 WiFi layer thinks the station interface is currently connected.
    err = Internal::CYW955913Utils::IsStationConnected(stationConnected);
    SuccessOrExit(err);
    ...
    if (stationConnected)
    {
            ...
            (mWiFiStationMode != kWiFiStationMode_Enabled || !IsWiFiStationProvisioned()))
        {
            ...
            err = Internal::CYW955913Utils::cyw955913_wifi_disconnect();
            if (err != CHIP_NO_ERROR)
            {
                ChipLogError(DeviceLayer, "cyw955913_wifi_disconnect() failed: %s", chip::ErrorStr(err));
            }
            SuccessOrExit(err);
    ...
    else
    {
        ...
        if (mWiFiStationMode == kWiFiStationMode_Enabled && IsWiFiStationProvisioned())
        {
                ...
                now >= mLastStationConnectFailTime + mWiFiStationReconnectInterval)
            {
                ...
                err = Internal::CYW955913Utils::cyw955913_wifi_connect();
                if (err != CHIP_NO_ERROR)
                {
                    ChipLogError(DeviceLayer, "cyw955913_wifi_connect() failed: %s", chip::ErrorStr(err));

                    // Otherwise arrange another connection attempt in 30 seconds in case the AP is currently unavailable
                    if (err == CHIP_ERROR(CY_RSLT_WCM_SECURITY_NOT_FOUND))
                    {
                        System::Clock::Timeout timeToNextConnect = mWiFiStationReconnectInterval;
...
void ConnectivityManagerImpl::UpdateInternetConnectivityState(void)
{
    ...
    IPAddress addr;
    bool stationConnected;
    Internal::CYW955913Utils::IsStationConnected(stationConnected);
    ...
    {
        net_interface = (NX_IP *)cy_network_get_nw_interface(CY_NETWORK_WIFI_STA_INTERFACE, 0);
        if (net_interface != NULL && nx_ip_interface_status_check(net_interface, 0, NX_IP_LINK_ENABLED, &temp, NX_NO_WAIT) == NX_SUCCESS)
        {
#if INET_CONFIG_ENABLE_IPV4
            if (nx_ip_interface_status_check(net_interface, 0, NX_IP_ADDRESS_RESOLVED, &temp, NX_NO_WAIT) == NX_SUCCESS)
            {
                haveIPv4Conn = true;
                ...
                ULONG ipv4_address;
                ULONG netmask;
                nx_ip_address_get(net_interface, &ipv4_address, &netmask);
                inet_ntop4((const unsigned char *)&ipv4_address, addr_str, sizeof(addr_str));
                ChipLogProgress(DeviceLayer, "IPv4 Address Assigned : %s", addr_str);
            }
#endif
            // Search among the IPv6 addresses assigned to the interface for a Global Unicast
            // address (2000::/3) that is in the valid state.  If such an address is found...
            for (uint8_t i = 0; i < NX_MAX_IPV6_ADDRESSES; i++)
            {
                UINT ipv6_status = nxd_ipv6_address_get(net_interface, i, &ipv6_address, &prefix_length, &interface_index);
                ULONG address_type = IPv6_Address_Type(ipv6_address.nxd_ip_address.v6);
                if (ipv6_status == NX_SUCCESS && (address_type & IPV6_ADDRESS_LINKLOCAL))
                {
                    haveIPv6Conn = true;
                    ChipDeviceEvent event;
                    event.Type                           = DeviceEventType::kInterfaceIpAddressChanged;
                    ...
                    NX_IPV6_ADDRESS_CHANGE_ENDIAN(ipv6_address.nxd_ip_address.v6);
                    inet_ntop6((const unsigned char *)ipv6_address.nxd_ip_address.v6, addr_str, sizeof(addr_str));
                    ChipLogProgress(DeviceLayer, "IPv6 Address Assigned : %s", addr_str);
                    break;
                }
            }
        }
...
CHIP_ERROR ConnectivityManagerImpl::WiFi_init(void)
{
    CHIP_ERROR err   = CHIP_NO_ERROR;
    cy_rslt_t result = CY_RSLT_SUCCESS;
    err              = Internal::CYW955913Utils::StartWiFiLayer();
    SuccessOrExit(err);
...
{
    CHIP_ERROR err = CHIP_NO_ERROR;
    err            = Internal::CYW955913Utils::ping_init();
    return err;
}
