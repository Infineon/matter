...
/**
 * Provides a generic implementation of ThreadStackManager features that works on ThreadX platforms.
 *
 * This template contains implementations of select features from the ThreadStackManager abstract
 * interface that are suitable for use on ThreadX-based platforms.  It is intended to be
...
class GenericThreadStackManagerImpl_ThreadX
{
...
protected:
    ...
    TX_THREAD mThreadTask;
    TX_EVENT_FLAGS_GROUP mEventFlags;
    TX_MUTEX mThreadStackLockMutex;
...
private:
    ...
    ULONG mThreadStack[CHIP_DEVICE_CONFIG_THREAD_TASK_STACK_SIZE];

};

// Instruct the compiler to instantiate the template only when explicitly told to do so.
extern template class GenericThreadStackManagerImpl_ThreadX<ThreadStackManagerImpl>;
