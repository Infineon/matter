...
CHIP_ERROR CommissioningWindowManager::OpenCommissioningWindow(Seconds32 commissioningTimeout)
{
    ...
    // For platform CYW955913EVK_01 we need to subtract 200ms from the commissioning window timeout due to
    // hardware limitations with ticks frequency of 10 ms, to be sure that closed commissioning window event will be sent on time
    // in a Test Harness (TH) environment only as the TH runs with a more accurate clock.
#ifdef CYW955913EVK_01
    auto commissioningTimeoutMs = std::chrono::duration_cast<chip::System::Clock::Milliseconds32>(commissioningTimeout);
    commissioningTimeoutMs -= chip::System::Clock::Milliseconds32(200);

    ReturnErrorOnFailure(DeviceLayer::SystemLayer().StartTimer(commissioningTimeoutMs, HandleCommissioningWindowTimeout, this));
#else
